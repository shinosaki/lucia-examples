...
import { createUnstorage, createUserDB } from '@/lib/db'
...
app.use('*',
  (c, next) => {
    const storage = createUnstorage(c.env.KV)
    c.set('storage', storage)
    c.set('db', createUserDB(storage))
    c.set('lucia', createLucia(storage, c.env.DEV))
    return next()
  },
  layout,
  csrf(),
